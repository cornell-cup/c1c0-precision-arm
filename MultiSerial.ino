...
void loop() {    
  ...
  float angle6 = pos_str.substring(idx_begin, idx_comma).toFloat();
  idx_begin = idx_comma+1;
  idx_comma = pos_str.indexOf(',', idx_begin);
  float angle5 = pos_str.substring(idx_begin, idx_comma).toFloat();
  idx_begin = idx_comma+1;
  idx_comma = pos_str.indexOf(',', idx_begin);
  float angle4 = pos_str.substring(idx_begin, idx_comma).toFloat();
  idx_begin = idx_comma+1;
  idx_comma = pos_str.indexOf(',', idx_begin);
  float angle3 = pos_str.substring(idx_begin, idx_comma).toFloat();
  idx_begin = idx_comma+1;
  float angle2 = pos_str.substring(idx_begin, idx_end).toFloat();
